...
export function CollapsibleDemo({ repos }: {repos: Repos[]}) {
  ...
  return (
    ...
    >
      ...
      {
        repos.length != 0 && repos.sort((repo1, repo2) => repo2.name.localeCompare(repo1.name)).slice(1, 2).map(repo => (
      ...
      {
        repos.length != 0 && repos.sort((repo1, repo2) => repo2.name.localeCompare(repo1.name)).slice(2, repos.length-1).map(repo => (
            <>
